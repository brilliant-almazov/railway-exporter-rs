...
impl Metrics {
    fn new() -> Self {
        ...
        let days_in_billing_period = GaugeVec::new(
            Opts::new("railway_days_in_billing_period", "Days elapsed in billing period"),
            &["project"],
        )
        .unwrap();
        ...
        let last_scrape_timestamp = GaugeVec::new(
            Opts::new("railway_exporter_last_scrape_timestamp", "Timestamp of last successful scrape"),
            &["project"],
        )
        .unwrap();
        let scrape_duration_seconds = GaugeVec::new(
            Opts::new("railway_exporter_scrape_duration_seconds", "Duration of API scrape"),
            &["project"],
        )
        .unwrap();
        ...
        registry.register(Box::new(days_in_billing_period.clone())).unwrap();
        registry.register(Box::new(days_remaining_in_month.clone())).unwrap();
        registry.register(Box::new(last_scrape_timestamp.clone())).unwrap();
        registry.register(Box::new(scrape_duration_seconds.clone())).unwrap();
